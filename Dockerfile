...
RUN python -m venv /opt/vectordb-env
ENV PATH /opt/vectordb-env/bin:$PATH
ENV VIRTUAL_ENV /opt/vectordb-env

# Set the working directory.
WORKDIR /workspaces/vectordb/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/vectordb/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/vectordb/ && touch src/vectordb/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/vectordb/
